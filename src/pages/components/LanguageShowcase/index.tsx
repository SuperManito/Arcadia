...
const iconLogoItems: LogoIconItem[] = [
...
function LanguageLogos () {
  ...
  return (
    <div className="mb-4 md:mb-12 md:mt-8 flex justify-center">
      <div className="flex flex-wrap justify-center items-center gap-4 sm:gap-6 md:gap-8 bg-neutral-50 dark:bg-neutral-900/60 border border-neutral-200 dark:border-neutral-800 rounded-2xl px-6 sm:px-10 py-6 sm:py-8 max-w-3xl sm:max-w-full">
        <div>
        ...
        {iconLogoItems.map((item) => (
          <div key={item.title}>
            <Link href={item.href} title={item.title} target="_blank" rel="noreferrer">
              <Icon
                icon={item.icon}
                size={isMobile ? item.mobileSize : item.desktopSize}
                style={(item.mobileMarginTop || item.desktopMarginTop) ? { marginTop: isMobile ? item.mobileMarginTop : item.desktopMarginTop } : undefined}
              />
            </Link>
          </div>
        ))}
...
export default function LanguageShowcase () {
  const fileExtensions = ['js', 'ts', 'py', 'go', 'rs', 'lua', 'rb', 'pl', 'c', 'sh']
    ...
    <div className="py-12 sm:py-24 bg-white dark:bg-neutral-950">
      <section className="container">
        <div className="flex flex-col items-center justify-center h-64 lg:h-full rounded-2xl" style={{ margin: '0 auto' }}>
          <div className="mt-8 md:mt-16 md:mb-16 dark:text-(--ifm-color-emphasis-500) text-center">
            <BlurFade delay={0.25} inView>
              <span className="text-neutral-900 dark:text-white font-bold tracking-tighter text-[1.4rem] md:text-[2.5rem] inline-flex items-center">
            ...
            <BlurFade delay={0.25 * 3} inView className="mt-2 md:mt-4">
              <span className="text-neutral-600 dark:text-neutral-400 tracking-tighter text-[.9rem] md:text-[1.1rem] block">一键运行代码文件，支持多种语言类型</span>
            </BlurFade>
          </div>
