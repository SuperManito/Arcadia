...
function FeatureComponent ({
  ...
  return (
    <div
      className={cn(
        'flex flex-col lg:border-r py-4 max-[1921px]:py-2 relative group/feature dark:border-neutral-800',
    ...
    >
      ...
      <div className="mb-4 max-[1921px]:mb-2 relative z-10 px-10 text-neutral-600 dark:text-neutral-400 group-hover/feature:text-[var(--ifm-color-primary)]">
        {icon}
      </div>
      ...
      <p className="text-sm text-neutral-400 dark:text-neutral-400 max-w-xs relative pt-6 max-[1921px]:pt-2 z-10 px-10">
        {description}
      </p>
    </div>
  )
  ...
  const features = [
    {
      ...
      icon: <Icon icon="ci:window-code-block" size={32} />,
    },
    {
      title: '定时任务调度',
      description: (
    ...
    },
    {
      ...
      icon: <Icon icon="tabler:variable" size={32} />,
    },
    {
      title: '运行日志展示与查询',
      description: (
      ...
      icon: <Icon icon="mdi:text-box-search-outline" size={32} />,
    },
    {
      title: '文件可视化管理',
      description: (
        <>
          仿桌面系统的文件管理页面，设有平铺和列表两种视图，支持创建、删除、重命名、上传、下载、删除、编辑等操作。
        </>
      ),
      icon: <Icon icon="mdi:folder-open-outline" size={32} />,
    },
    {
      title: '代码内容对比',
      description: (
      ...
      icon: <Icon icon="mdi:file-compare" size={32} />,
    },
    {
      title: '代码在线调试',
      description: (
      ...
      icon: <Icon icon="mdi:script-text-play-outline" size={32} />,
    },
    {
      title: '内置终端命令行',
      description: (
        <>
          集成了基于 <b><Link className="text-neutral-400 dark:text-neutral-400" href="https://xtermjs.org/" target="_blank" rel="noreferrer">Xterm.js</Link></b> 的 WEB 终端工具，实时、流畅且支持标签页多开，日常使用无需手动连接至服务端。
        </>
      ),
      icon: <Icon icon="heroicons-outline:terminal" size={32} />,
    },
  ]
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 relative z-10 max-w-7xl mx-auto pt-10 max-[1921px]:pt-0">
