...
function command_cleanup_main() {
    ...
    ## 生成进程清单
    ps -axo pid,time,user,start,command | egrep -v " ps | egrep | grep -E |node server|server\.js|PM2| npm | egrep | perl | sed |tgbot|tee -a|bash|/src/shell/|tail -f /dev/null" | grep -E "[0-9][0-9]:[0-9][0-9]:[0-9][0-9] root" | egrep "\.js$|\.mjs$||\.cjs$|\.py$|\.ts$|\.go$|\.c$" >${FileProcessList}
