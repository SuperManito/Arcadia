...
目前支持运行 `js` `mjs` `cjs` `py` `ts` `go` `c` `sh` 共四种类型的代码文件，项目已默认预装了 `Node.js`、`Python` 的运行环境，后续会适配更多语言类型
...
并发的作用是同时运行多个任务，那么意味着使用并发运行单个任务是无意义的；后台运行则是针对单个任务的与并发相反  
项目CLI会根据运行的任务数量检测使用场景，届时会判定一些无意义的场景从而报错跳出，你需要加强对功能的理解  
例如分组运行和拆分运行就是多任务类型，你应该使用并发运行而不是后台运行

这两个命令选项的作用实际上是等价的，为了便于理解才分开设计，你应该按照实际任务运行数量来选择使用

### 关于并发任务的一些注意事项

在使用并发时，并发与默认运行单任务不同会将每个任务的运行日志存储在独特的日志文件中，会使用标记进行区分  
`g<index>` 代表分组运行时的标记、`e<index>` 代表拆分运行时的标记、`t<index>` 表示并发任务数

另外目前并发受限于底层工作原理，暂时没有控制并发线程的概念即全量并发，请合理使用以减少对系统造成的压力
