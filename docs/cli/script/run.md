...
| `-c`, `--concurrent`           | 并发线程数 | 否 | 指定同时运行的最大任务数量，选项后需跟正整数，需与并发运行同时使用 |
...
:::tip 默认为全量并发，请合理使用避免对系统造成的压力过大，可以指定并发线程数来限制同时运行的最大任务数量
:::


### 关于重组表达式的应用示例

:::info 在 Linux 系统命令行中有一些特殊的字符，例如 `;` `&` 等。如果目标复合变量的分隔符包含这些特殊字符，届时需用英文引号包裹以避免传递错误
:::
