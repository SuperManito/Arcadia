...
| `tags` | `string` | 否 | 任务标签过滤（仅限系统任务，具体通过下方 [获取标签列表](#获取标签列表) 获取） |
...
| `log_directory` | `string` | 否 | 日志目录路径 |
| `source_file` | `string` | 否 | 源码文件路径 |
...
:::tip 主动运行和按照定时计划自动触发运行的过程大致相同，不过要注意区分的是前端管理面板设计的调试功能，该调试仅执行定时任务记录项所存储的 `shell` 字段命令，在运行完毕后不会更新定时任务的运行状态、最后运行时间等关键信息。
:::
