...
| `data` | `object` | 数据对象（参考上方[获取分页接口响应](#获取分页)） |
...
返回数据对象数组（数据对象参考上方[获取分页接口响应](#获取分页)）
...
如果只创建一个变量则返回数据对象（参考上方[获取分页接口响应](#获取分页)），如果是批量创建则返回对象 `{ "count": <number> }`，其中 `count` 表示成功创建的数量
...
更新后的数据对象（参考上方[获取分页接口响应](#获取分页)），如果是批量更新则返回数组对象
