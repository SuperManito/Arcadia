...
可以在前端 “环境配置 - 环境变量” 管理面板页面进行可视化配置，也可以直接定义在 `config.sh` 主要配置文件中

下面具体介绍两种配置方式

### 1. 环境变量 · 管理面板

本项目对环境变量功能有着独特的功能设计，前端页面使用分页数据表格设计，变量类型分为 `普通变量` 与 `复合变量`  
复合变量是普通变量的高级应用，它可以将多个值分开管理最后合并成一个值，实现了更加灵活的配置以满足用户需求

该功能用户配置数据存储于用户数据库（文件）中，每次修改后都会在用户配置文件目录下自动生成 `env.sh` 批量声明脚本，以此便于底层命令的加载和调用
